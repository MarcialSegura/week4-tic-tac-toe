...
def startGame(startingPlayer, board):
    ...
    turn = startingPlayer #labeling turn for the first player
    for i in range(9):
        printBoard(board) #prints gameboard
        print('Turn for ' + turn + '. Move on which space?') #ask where to play
        move = input() #gives value of input to variable
        board[move] = turn #updates board
        if( checkWinner(board, 'X') ): #final winning move for X
            print('X wins!')
            break
        elif ( checkWinner(board, 'O') ): #final winning move for O
            print('O wins!')
            break

        if turn == 'X': #switches to O
            turn = 'O'
        else:           #switches to X
            turn = 'X'
